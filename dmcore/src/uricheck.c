...
int uri_validate_path(char *path_str,
		      const uint16_t max_depth, const uint16_t max_len)
{
	int len = 0;
	...
	if (*path_str == '/') {
		...
		case 0:
			// unlimited depth
			return uri_validate_path(path_str + 1, max_depth,
						 max_len);
		case 1:
			// we already parsed one segment and there is more
			return OMADM_SYNCML_ERROR_URI_TOO_LONG;
		default:
			return uri_validate_path(path_str + 1, max_depth - 1,
						 max_len);
		}
	}
		 ...
		 const char *uri, char **oNodeURI, char **oPropId)
{
	...
	if (max_total_len) {
		DMC_FAIL_ERR(uri_len > max_total_len,
			     OMADM_SYNCML_ERROR_URI_TOO_LONG);
	}

	DMC_FAIL_ERR(strstr(uri, OMADM_TOKEN_LIST) != NULL,
	...
	if (prop_str) {
		...
		DMC_FAIL_NULL(*oPropId,
			      strdup(prop_str + strlen(OMADM_TOKEN_PROP)),
	...
	DMC_FAIL_ERR((node_uri)[uri_len - 1] == '/',
	...
	if (path_str) {
		DMC_FAIL(uri_validate_path
			 (path_str, max_depth, max_segment_len));
	}

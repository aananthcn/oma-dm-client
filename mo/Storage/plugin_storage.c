...
static char *prv_str_cat(char *first, char *second)
{
	char *string;
			    ...
			    omadmtree_node_kind_t * oNodeType, void *iData)
{
	...
	if ((*oNodeType == OMADM_NODE_NOT_EXIST)
	    && !strcmp(iURI, PRV_BASE_URI))
...
static int prv_rootGetFN(dmtree_node_t * nodeP, void *iData)
{
	DMC_ERR_MANAGE;
	...
	if (strcmp(nodeP->uri, PRV_BASE_URI)) {
		DMC_FAIL(omadm_dmsettings_utils_node_exists
			 (settings, nodeP->uri, &type));
	} else {
	...
	case OMADM_NODE_IS_INTERIOR:
		{
			...
			DMC_FAIL(omadm_dmsettings_utils_get_node_children
				 (settings, nodeP->uri, &children));
			...
			DMC_FAIL_NULL(nodeP->format, strdup("node"),
				      OMADM_SYNCML_ERROR_DEVICE_FULL);
			nodeP->type = NULL;
		}
		break;
	case OMADM_NODE_IS_LEAF:
		{
			DMC_FAIL(omadm_dmsettings_utils_get_value
				 (settings, nodeP->uri, &nodeP->data_buffer));
			nodeP->data_size = strlen(nodeP->data_buffer) + 1;
			DMC_FAIL(omadm_dmsettings_utils_get_meta
				 (settings, nodeP->uri,
				  OMADM_NODE_PROPERTY_FORMAT, &nodeP->format));
			DMC_FAIL(omadm_dmsettings_utils_get_meta
				 (settings, nodeP->uri,
				  OMADM_NODE_PROPERTY_TYPE, &nodeP->type));
		}
		break;
	default:
...
static int prv_rootSetFN(const dmtree_node_t * nodeP, void *iData)
{
	DMC_ERR_MANAGE;
	dmsettings *settings = (dmsettings *) iData;

	if (!strcmp(nodeP->format, "node")) {
		DMC_FAIL(omadm_dmsettings_utils_create_non_leaf
			 (settings, nodeP->uri));
	} else {
		DMC_FAIL(omadm_dmsettings_utils_set_value
			 (settings, nodeP->uri, nodeP->data_buffer));
		if (nodeP->format) {
			DMC_FAIL(omadm_dmsettings_utils_set_meta
				 (settings, nodeP->uri,
				  OMADM_NODE_PROPERTY_FORMAT, nodeP->format));
		}
		if (nodeP->type) {
...
static int prv_rootGetACLFN(const char *iURI, char **oValue, void *iData)
{
	DMC_ERR_MANAGE;
	dmsettings *settings = (dmsettings *) iData;

	if (!strcmp(iURI, PRV_BASE_URI)) {
		DMC_FAIL_NULL(*oValue, strdup("Add=*&Get=*&Replace=*&Delete=*"),
			      OMADM_SYNCML_ERROR_DEVICE_FULL);
	} else {
		DMC_FAIL(omadm_dmsettings_utils_get_meta
			 (settings, iURI, OMADM_NODE_PROPERTY_ACL, oValue));
	}
...
static int prv_rootSetACLFN(const char *iURI, const char *acl, void *iData)
{
	dmsettings *settings = (dmsettings *) iData;

	return omadm_dmsettings_utils_set_meta(settings, iURI,
					       OMADM_NODE_PROPERTY_ACL, acl);
}

static int prv_rootDeleteFN(const char *iURI, void *iData)
{
	dmsettings *settings = (dmsettings *) iData;

...
extern Ret_t smlLockWriteBuffer(InstanceID_t id, MemPtr_t * pWritePosition,
				MemSize_t * freeSize);
...
static Ret_t mgrCreateNextCommand(InstanceID_t id,
				 ...
				 SmlVersion_t vers)
{
	...
	rc = smlUnlockWriteBuffer(id,
				  (MemSize_t) pCurrentWritePosition -
...
SML_API Ret_t smlEndMessage(InstanceID_t id, Boolean_t final)
{
	...
	/* --- Call the encoder module --- */
	rc = xltEncTerminate(pInstanceInfo->encoderState,
			     pCurrentWritePosition + freeSize,
			     &pCurrentWritePosition);
...
SML_API Ret_t smlStartEvaluation(InstanceID_t id)
{
	...
	rc = xltStartEvaluation((XltEncoderPtr_t)
				(pInstanceInfo->encoderState));
 ...
 */
static Ret_t mgrCreateNextCommand(InstanceID_t id,
				  SmlProtoElement_t cmdType, VoidPtr_t pContent)
{
	/* --- Definitions --- */
	InstanceInfoPtr_t pInstanceInfo;	// pointer the the instance info structure for this id
	Ret_t rc;
	...
	/* --- Call the encoder module --- */
	rc = xltEncAppend(pInstanceInfo->encoderState, cmdType,
	...
	if (rc != SML_ERR_OK) {
		...
		if (rc == SML_ERR_XLT_BUF_ERR) {
			...
			if (pInstanceInfo->callbacks->transmitChunkFunc != NULL) {
				...
				smlUnlockWriteBuffer(id, (MemSize_t) 0);
				// call the callback
				pInstanceInfo->callbacks->transmitChunkFunc(id,
									    NULL);
				// lock -> returns the amount of free buffer space
				smlLockWriteBuffer(id, &pCurrentWritePosition,
						   &freeSize);
				pBeginPosition = pCurrentWritePosition;
				// now try again to encode and see wether we now have enough mem available
				rc = xltEncAppend(pInstanceInfo->encoderState,
						  cmdType,
						  pCurrentWritePosition +
						  freeSize, pContent,
						  &pCurrentWritePosition);

...
SML_API Ret_t smlInitInstance(SmlCallbacksCPtr_t callbacks,
			      SmlInstanceOptionsPtr_t pOptions,
			      VoidPtr_t pUserData, InstanceID_t * pInstanceID);
SML_API Ret_t smlTerminateInstance(InstanceID_t id);
SML_API Ret_t smlLockReadBuffer(InstanceID_t id, MemPtr_t * pReadPosition,
				MemSize_t * usedSize);
...
#endif
SML_API Ret_t smlLockWriteBuffer(InstanceID_t id,
				 MemPtr_t * pWritePosition,
				 MemSize_t * freeSize);
 ...
 * @return Error Code
 */
SML_API Ret_t smlInitInstance(SmlCallbacksCPtr_t pCallbacks,
			      SmlInstanceOptionsPtr_t pOptions,
			      VoidPtr_t pUserData, InstanceID_t * pInstanceID)
{
	...
	if ((rc =
	     wsmCreate(pOptions->workspaceName, pOptions->workspaceSize,
	...
	pInstanceInfo = (InstanceInfoPtr_t) smlLibMalloc((MemSize_t)
							 sizeof
							 (InstanceInfo_t));
	if (pInstanceInfo == NULL) {
#ifndef NOWSM
	...
	pInstanceInfo->instanceBuffer =
	    smlLibMalloc(pInstanceInfo->instanceBufSiz);
...
SML_API Ret_t smlSetCallbacks(InstanceID_t id, SmlCallbacksCPtr_t pCallbacks)
{
	...
	pCallbacksCopy = (SmlCallbacksPtr_t) smlLibMalloc((MemSize_t)
							  sizeof
							  (SmlCallbacks_t));
	if (pCallbacksCopy == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	smlLibMemcpy(pCallbacksCopy, pCallbacks,
		     (MemSize_t) sizeof(SmlCallbacks_t));
...
SML_API Ret_t smlUnlockReadBuffer(InstanceID_t id, MemSize_t processedBytes)
{
	...
	if (pInstanceInfo->readPointer + processedBytes >
	    pInstanceInfo->writePointer)
...
SML_API Ret_t smlReadOutgoingAgain(InstanceID_t id)
{
	...
	if (pInstanceInfo->outgoingMsgStart == NULL)
		return SML_ERR_WRONG_USAGE;	// no outgoing start set
				   ...
				   MemPtr_t * message, MemSize_t * msgsize)
{
	...
	if (*message >= pInstanceInfo->writePointer)
		return SML_ERR_WRONG_USAGE;	// invalid pointer positions
				 ...
				 MemSize_t * freeSize)
{
	...
	*pWritePosition = pInstanceInfo->writePointer;
	...
	if (pInstanceInfo->maxOutgoingSize &&
	    pInstanceInfo->outgoingMsgStart &&
	    pInstanceInfo->outgoingMsgStart < pInstanceInfo->writePointer) {
		// calculate what is allowed according to maxOutgoingSize
		*freeSize = (pInstanceInfo->maxOutgoingSize) -	// maximum outgoing size
		    (pInstanceInfo->writePointer - pInstanceInfo->outgoingMsgStart);	// size of outgoing message so far
		if (pInstanceInfo->writePointer + *freeSize >
		    pInstanceInfo->instanceBuffer +
		    pInstanceInfo->instanceBufSiz) {
			// actual space in buffer is smaller
			*freeSize = (pInstanceInfo->instanceBuffer + pInstanceInfo->instanceBufSiz) -	// end of buffer
...
SML_API Ret_t smlUnlockWriteBuffer(InstanceID_t id, MemSize_t writtenBytes)
{
	...
	if (writtenBytes > 0) {
		...
		if (pInstanceInfo->writePointer + writtenBytes >
		    pInstanceInfo->instanceBuffer +
		    pInstanceInfo->instanceBufSiz)
...
Ret_t setInstanceOptions(InstanceID_t id, SmlInstanceOptionsPtr_t pOptions)
{
	...
	pOptionsCopy = (SmlInstanceOptionsPtr_t) smlLibMalloc((MemSize_t)
							      sizeof
							      (SmlInstanceOptions_t));
	if (pOptionsCopy == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	smlLibMemcpy(pOptionsCopy, pOptions,
		     (MemSize_t) sizeof(SmlInstanceOptions_t));

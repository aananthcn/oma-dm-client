...
SML_API Ret_t smlInit(SmlOptionsPtr_t pCoreOptions)
{
	...
	pGlobalAnchor =
	    (SyncMLInfoPtr_t) smlLibMalloc((MemSize_t) sizeof(SyncMLInfo_t));
	if (pGlobalAnchor == NULL)
	...
	pGlobalAnchor->tokTbl =
	    (TokenInfoPtr_t) smlLibMalloc(sizeof(TokenInfo_t));
	...
	pWorkspaceOptions->maxAvailMem =
	    (MemSize_t) pGlobalAnchor->syncmlOptions->maxWorkspaceAvailMem;
...
SML_API Ret_t smlSetSyncMLOptions(SmlOptionsPtr_t pCoreOptions)
{
	...
	pCoreOptionsCopy =
	    (SmlOptionsPtr_t) smlLibMalloc((MemSize_t) sizeof(SmlOptions_t));
	if (pCoreOptionsCopy == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	smlLibMemcpy(pCoreOptionsCopy, pCoreOptions,
		     (MemSize_t) sizeof(SmlOptions_t));

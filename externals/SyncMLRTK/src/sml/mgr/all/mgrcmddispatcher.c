...
#endif
Ret_t smlLockReadBuffer(InstanceID_t id, MemPtr_t * pReadPosition,
			MemSize_t * usedSize);
...
static Ret_t mgrProcessNextCommand(InstanceID_t id,
				   InstanceInfoPtr_t pInstanceInfo);
static Ret_t mgrProcessStartMessage(InstanceID_t id,
				    InstanceInfoPtr_t pInstanceInfo);
				    ...
				    InstanceInfoPtr_t pInstanceInfo)
{
	...
	rc = xltDecInit(pInstanceInfo->instanceOptions->encoding,
			pCurrentReadPosition + usedSize - 1,
			&pCurrentReadPosition,
			(XltDecoderPtr_t *) & (pInstanceInfo->decoderState),
			&pContent);
	...
	rc = smlUnlockReadBuffer(id,
				 (MemSize_t) pCurrentReadPosition -
				 (MemSize_t) pBeginPosition);
	if (rc != SML_ERR_OK)
		return rc;
	...
	if (pInstanceInfo->callbacks->startMessageFunc == NULL)
		return SML_ERR_COMMAND_NOT_HANDLED;
	rc = pInstanceInfo->callbacks->startMessageFunc(id,
				   ...
				   InstanceInfoPtr_t pInstanceInfo)
{
	...
	/* --- Parse next Command --- */
	rc = xltDecNext(pInstanceInfo->decoderState,
			pCurrentReadPosition + usedSize, &pCurrentReadPosition,
			&cmdType, &pContent);
	...
	case SML_PE_ADD:
		if (pInstanceInfo->callbacks->addCmdFunc == NULL)
			return SML_ERR_COMMAND_NOT_HANDLED;
		return pInstanceInfo->callbacks->addCmdFunc(id,
							    pInstanceInfo->
							    userData, pContent);
		break;
	...
	case SML_PE_PUT:
		if (pInstanceInfo->callbacks->putCmdFunc == NULL)
			return SML_ERR_COMMAND_NOT_HANDLED;
		return pInstanceInfo->callbacks->putCmdFunc(id,
							    pInstanceInfo->
							    userData, pContent);
		break;
	...
	case SML_PE_GET:
		if (pInstanceInfo->callbacks->getCmdFunc == NULL)
			return SML_ERR_COMMAND_NOT_HANDLED;
		return pInstanceInfo->callbacks->getCmdFunc(id,
							    pInstanceInfo->
							    userData, pContent);
		break;
	...
	case SML_PE_MAP:
		if (pInstanceInfo->callbacks->mapCmdFunc == NULL)
			return SML_ERR_COMMAND_NOT_HANDLED;
		return pInstanceInfo->callbacks->mapCmdFunc(id,
							    pInstanceInfo->
							    userData, pContent);
		break;
#endif
	...
	case SML_PE_SEARCH:
		if (pInstanceInfo->callbacks->searchCmdFunc == NULL)
			return SML_ERR_COMMAND_NOT_HANDLED;
		return pInstanceInfo->callbacks->searchCmdFunc(id,
							       pInstanceInfo->
							       userData,
							       pContent);
		break;
	...
	case SML_PE_COPY:
		...
		/* Handle MOVE Command */
	case SML_PE_MOVE:
		if (pInstanceInfo->callbacks->moveCmdFunc == NULL)
			return SML_ERR_COMMAND_NOT_HANDLED;
		return pInstanceInfo->callbacks->moveCmdFunc(id,
							     pInstanceInfo->
							     userData,
							     pContent);
		break;

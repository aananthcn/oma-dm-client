...
{
	if (aRc >= SML_ERR_XLT_MISSING_CONT
	    && aRc <= SML_ERR_XLT_INVAL_INPUT_DATA) {
...
void show_token(XltDecTokenPtr_t aToken, uInt32 aDebugMask)
{
 ...
 foundtag:
		...
		// content
		SMLERRPRINTFX(aDebugMask,
			      ("- parsing content (%s token, ext=%s)", toktype,
			       extname));
	} else {
		// unknown
		SMLERRPRINTFX(aDebugMask,
			      ("- parsing %s token, ext=%s", toktype, extname));
	}
}				// show_token

Ret_t show_decode_error(Ret_t aRc, XltDecScannerPtr_t aScanner,
			char *aRoutineName)
{
	...
	if (aRc != SML_ERR_OK) {
		...
		SMLERRPRINTFX(DBG_ERROR,
			      ("%s: smlErr 0x%hX (%s) while parsing",
			       aRoutineName, aRc, smlErrorText(aRc)));
		// Show details
 ...
 */
static SmlPcdataPtr_t concatPCData(SmlPcdataPtr_t pDat1,
 ...
 *         else error code
 */
static Ret_t appendItemList(XltDecoderPtr_t pDecoder,
			    SmlItemListPtr_t * ppItemList);
#ifdef SEARCH_RECEIVE
static Ret_t appendSourceList(XltDecoderPtr_t pDecoder,
			      SmlSourceListPtr_t * ppSourceList);
#endif
#ifdef MAPITEM_RECEIVE
static Ret_t appendMapItemList(XltDecoderPtr_t pDecoder,
			       SmlMapItemListPtr_t * ppMapItemList);
#endif
static Ret_t appendTargetRefList(XltDecoderPtr_t pDecoder,
				 SmlTargetRefListPtr_t * ppTargetRefList);
static Ret_t appendSourceRefList(XltDecoderPtr_t pDecoder,
...
xltDecInit(const SmlEncoding_t enc,
	   ...
	   MemPtr_t * ppBufPos,
	   XltDecoderPtr_t * ppDecoder, SmlSyncHdrPtr_t * ppSyncHdr)
{
	...
	if ((pDecoder =
	     (XltDecoderPtr_t) smlLibMalloc(sizeof(XltDecoder_t))) == NULL)
...
xltDecNext(XltDecoderPtr_t pDecoder,
	   const MemPtr_t pBufEnd,
	   MemPtr_t * ppBufPos, SmlProtoElement_t * pe, VoidPtr_t * ppContent)
{
	...
	if ((rc =
	     pDecPriv->tagstack->top(pDecPriv->tagstack, &tagid)) != SML_ERR_OK)
		return rc;
	if (tagid == TN_SYNCML) {
		...
		if (!((IS_START(pScanner->curtok)) &&
			...
			return SML_DECODEERROR(SML_ERR_XLT_INVAL_PROTO_ELEM,
					       pScanner, "xltDecNext");
		}
	}
	...
	if ((IS_START_OR_EMPTY(pScanner->curtok))
		...
		while ((cPE_Dec[i].tagid) != TN_UNDEF) {
			if ((cPE_Dec[i].tagid) == pScanner->curtok->tagid) {
				*pe = (cPE_Dec[i].type);
				if ((rc =
				     cPE_Dec[i].build(pDecPriv,
						      ppContent)) != SML_ERR_OK)
					return rc;
				...
				if (((SmlGenericCmdPtr_t) (*ppContent))->
				    elementType == SML_PE_GENERIC) {
					((SmlGenericCmdPtr_t) (*ppContent))->
					    elementType = cPE_Dec[i].type;
				}
		...
		case TN_SYNCBODY:
			...
			if ((pScanner->curtok->type == TOK_TAG_END) &&
				...
				*ppContent = NULL;
				...
				return
				    SML_DECODEERROR
				    (SML_ERR_XLT_INVAL_SYNCML_DOC, pScanner,
				     "xltDecNext");
			}
			break;
		default:
...
Ret_t discardToken(XltDecoderPtr_t pDecoder)
{
	...
	if ((rc = pDecoder->scanner->pushTok(pDecoder->scanner)) != SML_ERR_OK)
		return rc;
	if ((rc =
	     pDecoder->tagstack->pop(pDecoder->tagstack, &tmp)) != SML_ERR_OK)
		return rc;
	return SML_ERR_OK;
}
...
concatPCData(SmlPcdataPtr_t pDat1, const SmlPcdataPtr_t pDat2)
{
	...
	case SML_PCDATA_OPAQUE:
		...
		if ((pDat1->content =
		     smlLibRealloc(pDat1->content,
				   pDat1->length + pDat2->length + 1)) == NULL)
			return NULL;
		smlLibMemmove(((Byte_t *) pDat1->content) + pDat1->length,
...
Ret_t buildSyncHdr(XltDecoderPtr_t pDecoder, VoidPtr_t * ppSyncHdr)
{
	XltDecScannerPtr_t pScanner;
	SmlSyncHdrPtr_t pSyncHdr;
	Ret_t rc;
	Long_t sessionid = 0, msgid = 0, source = 0, target = 0, version =
	    0, proto = 0;
	...
	if ((pSyncHdr =
	     (SmlSyncHdrPtr_t) smlLibMalloc(sizeof(SmlSyncHdr_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_VERSION:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pSyncHdr->version);
			version++;
			break;
		case TN_PROTO:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pSyncHdr->proto);
			proto++;
			break;
		case TN_SESSIONID:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pSyncHdr->sessionID);
			sessionid++;
			break;
		case TN_MSGID:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pSyncHdr->msgID);
			msgid++;
			break;
		case TN_RESPURI:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pSyncHdr->respURI);
			break;

			/* child tags */
		case TN_TARGET:
			rc = buildTargetOrSource(pDecoder,
						 (VoidPtr_t) & pSyncHdr->
						 target);
			target++;
			break;
...
Ret_t buildSync(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	while ((pScanner->curtok->type != TOK_TAG_END) && !break_sync) {
		...
		case TN_TARGET:
			rc = buildTargetOrSource(pDecoder,
						 (VoidPtr_t) & pSync->target);
			break;
		case TN_SOURCE:
			rc = buildTargetOrSource(pDecoder,
						 (VoidPtr_t) & pSync->source);
			break;
	...
	if (!break_sync) {
		if ((pScanner->curtok->tagid) != TN_SYNC) {
			...
			if ((rc =
			     pDecoder->scanner->pushTok(pDecoder->scanner)) !=
			    SML_ERR_OK) {
				smlFreeSync(pSync);
				return rc;
			}
...
Ret_t buildGenericCmd(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pGenCmd =
	     (SmlGenericCmdPtr_t) smlLibMalloc(sizeof(SmlGenericCmd_t))) ==
	    NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t buildMap(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_SOURCE:
			rc = buildTargetOrSource(pDecoder,
						 (VoidPtr_t) & pMap->source);
			source++;
			break;
		case TN_TARGET:
			rc = buildTargetOrSource(pDecoder,
						 (VoidPtr_t) & pMap->target);
			target++;
			break;
#ifdef MAPITEM_RECEIVE
			/* Lists */
...
#ifdef SEARCH_RECEIVE
Ret_t buildSearch(XltDecoderPtr_t pDecoder, VoidPtr_t * ppSearch)
{
	XltDecScannerPtr_t pScanner;
	...
	if ((pSearch =
	     (SmlSearchPtr_t) smlLibMalloc(sizeof(SmlSearch_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t
buildRecordFieldFilter(XltDecoderPtr_t pDecoder,
		       VoidPtr_t * ppRecordFieldFilter)
{
	XltDecScannerPtr_t pScanner;
	...
	if (*ppRecordFieldFilter != NULL)
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC, pScanner,
				       "buildRecordFieldFilter");

	if ((pRecordFieldFilter = (SmlRecordOrFieldFilterPtr_t)
	     smlLibMalloc(sizeof(SmlRecordOrFieldFilter_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		switch (pScanner->curtok->tagid) {
			...
			/* Item */
		case TN_ITEM:
			rc = buildItem(pDecoder,
				       (VoidPtr_t) & pRecordFieldFilter->item);
			item++;
			break;
...
Ret_t buildFilter(XltDecoderPtr_t pDecoder, VoidPtr_t * ppFilter)
{
	XltDecScannerPtr_t pScanner;
	...
	if ((pFilter =
	     (SmlFilterPtr_t) smlLibMalloc(sizeof(SmlFilter_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_FIELD:
			rc = buildRecordFieldFilter(pDecoder,
						    (VoidPtr_t) & pFilter->
						    field);
			break;
		...
		case TN_FILTERTYPE:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pFilter->filtertype);
			break;

		default:
			rc = SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					     pScanner, "buildFilter");
		}
		if (rc != SML_ERR_OK) {
...
buildTargetOrSourceParent(XltDecoderPtr_t pDecoder, VoidPtr_t * ppTargetParent)
{
	...
	if (*ppTargetParent != NULL)
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC, pScanner,
				       "buildTargetOrSourceParent");

	if ((pTargetParent =
	     (SmlTargetParentPtr_t) smlLibMalloc(sizeof(SmlTargetParent_t))) ==
	    NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_LOCURI:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pTargetParent->locURI);
			locuri++;
			break;
		default:
			rc = SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
...
Ret_t buildTargetOrSource(XltDecoderPtr_t pDecoder, VoidPtr_t * ppTarget)
{
	XltDecScannerPtr_t pScanner;
	...
	if ((pTarget =
	     (SmlTargetPtr_t) smlLibMalloc(sizeof(SmlTarget_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_LOCURI:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pTarget->locURI);
			locuri++;
			break;
		case TN_LOCNAME:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pTarget->locName);
			break;
			/* SyncML 1.2 Filter element, %%% added 2005-08-17 by synthesis/luz */
		case TN_FILTER:
			rc = buildFilter(pDecoder,
					 (VoidPtr_t) & pTarget->filter);
			break;
		default:
			rc = SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					     pScanner,
...
Ret_t buildChal(XltDecoderPtr_t pDecoder, VoidPtr_t * ppChal)
{
	XltDecScannerPtr_t pScanner;
	SmlChalPtr_t pChal;
	Long_t meta = 0;
...
Ret_t buildCred(XltDecoderPtr_t pDecoder, VoidPtr_t * ppCred)
{
	XltDecScannerPtr_t pScanner;
	SmlCredPtr_t pCred;
	Ret_t rc;
...
Ret_t buildItem(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
...
#ifdef __SML_WBXML__
			if (pItem->data
			    && pItem->data->contentType == SML_PCDATA_OPAQUE)
				subdtdDecodeWbxml(pDecoder,
						  (SmlPcdataPtr_t *) & pItem->
						  data);
#endif
#endif
			break;
		...
		case TN_TARGET:
			rc = buildTargetOrSource(pDecoder,
						 (VoidPtr_t) & pItem->target);
			break;
		case TN_SOURCE:
			rc = buildTargetOrSource(pDecoder,
						 (VoidPtr_t) & pItem->source);
			break;
		case TN_TARGETPARENT:
			rc = buildTargetOrSourceParent(pDecoder,
						       (VoidPtr_t) &
						       pItem->targetParent);
			break;
...
Ret_t buildMapItem(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pMapItem =
	     (SmlMapItemPtr_t) smlLibMalloc(sizeof(SmlMapItem_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t buildStatus(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pStatus =
	     (SmlStatusPtr_t) smlLibMalloc(sizeof(SmlStatus_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_CMDID:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pStatus->cmdID);
			cmdid++;
			break;
		case TN_MSGREF:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pStatus->msgRef);
			break;
		case TN_CMDREF:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pStatus->cmdRef);
			break;
		case TN_CMD:
		...
		case TN_CRED:
			...
			/* Lists */
		case TN_ITEM:
			rc = appendItemList(pDecoder,
					    (VoidPtr_t) & pStatus->itemList);
			break;
		case TN_TARGETREF:
			rc = appendTargetRefList(pDecoder,
						 (VoidPtr_t) & pStatus->
						 targetRefList);
			break;
		case TN_SOURCEREF:
			rc = appendSourceRefList(pDecoder,
						 (VoidPtr_t) & pStatus->
						 sourceRefList);
			break;
...
Ret_t buildResults(XltDecoderPtr_t pDecoder, VoidPtr_t * ppResults)
{
	XltDecScannerPtr_t pScanner;
	...
	if ((pResults =
	     (SmlResultsPtr_t) smlLibMalloc(sizeof(SmlResults_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_CMDID:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pResults->cmdID);
			cmdid++;
			break;
		case TN_MSGREF:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pResults->msgRef);
			break;
		case TN_CMDREF:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pResults->cmdRef);
			cmdref++;
			break;
		case TN_META:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pResults->meta);
			break;
		case TN_TARGETREF:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pResults->targetRef);
			break;
		case TN_SOURCEREF:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pResults->sourceRef);
			break;
...
Ret_t buildEmptyTag(XltDecoderPtr_t pDecoder)
{
	...
	if (!IS_END(pDecoder->scanner->curtok))
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
				       pDecoder->scanner, "buildEmptyTag");
	// yep, ok
	return SML_ERR_OK;
}

Ret_t buildPCData(XltDecoderPtr_t pDecoder, VoidPtr_t * ppPCData)
{
	XltDecScannerPtr_t pScanner;
	...
	if (IS_EMPTY(pScanner->curtok)) {
		if ((pPCData =
		     (SmlPcdataPtr_t) smlLibMalloc(sizeof(SmlPcdata_t))) ==
		    NULL)
			return SML_ERR_NOT_ENOUGH_SPACE;
	...
	if (IS_CONTENT(pScanner->curtok)) {
		...
		while (pScanner->curtok->type == TOK_CONT) {
			if (pPCData == NULL)
				...
				if (pPCData == NULL)
					return
					    SML_DECODEERROR
					    (SML_ERR_XLT_INVAL_PCDATA, pScanner,
					     "buildPCData_3");
			}
		...
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_PCDATA, pScanner,
				       "buildPCData_5");
	}
...
appendItemList(XltDecoderPtr_t pDecoder, SmlItemListPtr_t * ppItemList)
{
	...
	if ((pNewItemList =
	     (SmlItemListPtr_t) smlLibMalloc(sizeof(SmlItemList_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	smlLibMemset(pNewItemList, 0, sizeof(SmlItemList_t));

	if ((rc =
	     buildItem(pDecoder,
		       (VoidPtr_t) & pNewItemList->item)) != SML_ERR_OK) {
...
appendSourceList(XltDecoderPtr_t pDecoder, SmlSourceListPtr_t * ppSourceList)
{
	...
	if ((pNewSourceList =
	     (SmlSourceListPtr_t) smlLibMalloc(sizeof(SmlSourceList_t))) ==
	    NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	smlLibMemset(pNewSourceList, 0, sizeof(SmlSourceList_t));

	if ((rc =
	     buildTargetOrSource(pDecoder,
				 (VoidPtr_t) & pNewSourceList->source)) !=
	    SML_ERR_OK) {
...
appendMapItemList(XltDecoderPtr_t pDecoder, SmlMapItemListPtr_t * ppMapItemList)
{
	...
	if ((pNewMapItemList =
	     (SmlMapItemListPtr_t) smlLibMalloc(sizeof(SmlMapItemList_t))) ==
	    NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	smlLibMemset(pNewMapItemList, 0, sizeof(SmlMapItemList_t));

	if ((rc =
	     buildMapItem(pDecoder,
			  (VoidPtr_t) & pNewMapItemList->mapItem)) !=
	    SML_ERR_OK) {
...
static Ret_t
appendTargetRefList(XltDecoderPtr_t pDecoder,
		    SmlTargetRefListPtr_t * ppTargetRefList)
{
	...
	if ((pNewTargetRefList =
	     (SmlTargetRefListPtr_t) smlLibMalloc(sizeof(SmlTargetRefList_t)))
	    == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	smlLibMemset(pNewTargetRefList, 0, sizeof(SmlTargetRefList_t));

	if ((rc =
	     buildPCData(pDecoder,
			 (VoidPtr_t) & pNewTargetRefList->targetRef)) !=
	    SML_ERR_OK) {
...
static Ret_t
appendSourceRefList(XltDecoderPtr_t pDecoder,
		    SmlSourceRefListPtr_t * ppSourceRefList)
{
	...
	if ((pNewSourceRefList =
	     (SmlSourceRefListPtr_t) smlLibMalloc(sizeof(SmlSourceRefList_t)))
	    == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	if ((rc =
	     buildPCData(pDecoder,
			 (VoidPtr_t) & pNewSourceRefList->sourceRef)) !=
	    SML_ERR_OK) {

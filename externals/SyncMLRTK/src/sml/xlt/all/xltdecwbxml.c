...
struct wbxmlScannerPriv_s {
	...
	SmlPcdataExtension_t activeExt;
				    /**< the active Sub DTD */
};
		...
		XltDecScannerPtr_t * ppScanner)
{
	...
	if ((pScanner =
	     (wbxmlScannerPrivPtr_t) smlLibMalloc(sizeof(wbxmlScannerPriv_t)))
	    == NULL)
	...
	if ((pScanner->curtok =
	     (XltDecTokenPtr_t) smlLibMalloc(sizeof(XltDecToken_t))) == NULL) {
		smlLibFree(pScanner);
		*ppScanner = NULL;
		return SML_ERR_NOT_ENOUGH_SPACE;
	}
...
static Ret_t wbxmlVersion(wbxmlScannerPrivPtr_t pScanner)
{
	...
	if (major != _MAJOR_VERSION || minor > _MAX_PARSER_MINOR_VERSION)
		return SML_DECODEERROR(SML_ERR_XLT_INCOMP_WBXML_VERS, pScanner,
				       "wbxmlVersion");
...
static Ret_t wbxmlStrtbl(wbxmlScannerPrivPtr_t pScanner)
{
	...
	if (pScanner->pubID == 0) {
		if (pScanner->pubIDIdx > pScanner->strtbllen)
			return SML_DECODEERROR(SML_ERR_XLT_INVAL_WBXML_DOC,
					       pScanner, "wbxmlStrtbl");
		pScanner->pubIDStr =
		    (String_t) (pScanner->strtbl + pScanner->pubIDIdx);
	}
...
static Ret_t wbxmlStringToken(wbxmlScannerPrivPtr_t pScanner)
{
	...
	if ((pPcdata =
	     (SmlPcdataPtr_t) smlLibMalloc(sizeof(SmlPcdata_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	if (IS_STR_I(pScanner->pos)) {
		...
		if ((pPcdata->content =
		     smlLibMalloc(pPcdata->length + 1)) == NULL) {
			smlLibFree(pPcdata);
			return SML_ERR_NOT_ENOUGH_SPACE;
		}
		smlLibStrncpy(pPcdata->content, (String_t) pScanner->pos,
			      pPcdata->length + 1);
		...
		pPcdata->contentType = SML_PCDATA_STRING;
		pPcdata->length =
		    smlLibStrlen((String_t) (pScanner->strtbl + offset));
		if ((pPcdata->content =
		     smlLibMalloc(pPcdata->length + 1)) == NULL) {
			smlLibFree(pPcdata);
			return SML_ERR_NOT_ENOUGH_SPACE;
		}
		smlLibStrncpy(pPcdata->content,
			      (String_t) (pScanner->strtbl + offset),
			      pPcdata->length + 1);
...
static Ret_t wbxmlTagToken(wbxmlScannerPrivPtr_t pScanner)
{
	...
	if (IS_LITERAL(pScanner->pos)) {
		...
		rc = (Ret_t)
		    getTagIDByStringAndExt((String_t)
					   (pScanner->strtbl + offset),
					   pScanner->activeExt, &tagid);
		if ((tagid == TN_UNDEF) || (rc != SML_ERR_OK))
			return rc;

	} else {
		rc = (Ret_t) getTagIDByByteAndExt((Byte_t)
						  IDENTITY(pScanner->pos),
						  pScanner->activeExt, &tagid);
		if ((tagid == TN_UNDEF) || (rc != SML_ERR_OK))
			return rc;
	...
	if (has_cont) {
		if ((rc =
		     pScanner->tagstack->push(pScanner->tagstack,
					      tagid)) != SML_ERR_OK)
			return rc;
	}
	...
	if (IS_EXT(pScanner->pos)) {
		...
		if (!readBytes(pScanner, 1))
			return SML_DECODEERROR(SML_ERR_XLT_END_OF_BUFFER,
					       pScanner, "wbxmlSkipExtension");
		if (!readBytes
		    (pScanner, smlLibStrlen((String_t) pScanner->pos) + 1))
			return SML_DECODEERROR(SML_ERR_XLT_END_OF_BUFFER,
					       pScanner, "wbxmlSkipExtension");
	} else {
 ...
 */
static Ret_t wbxmlSkipPI(wbxmlScannerPrivPtr_t pScanner)
{
 ...
 * scanner recognizes and skips over them.
 */
static Ret_t wbxmlSkipAttribute(wbxmlScannerPrivPtr_t pScanner)
{
...
void subdtdDecodeWbxml(XltDecoderPtr_t pDecoder, SmlPcdataPtr_t * ppPcdata)
{
	...
	if (xltDecWbxmlInit
	    (pSubBuf + (*ppPcdata)->length, &pSubBuf,
	...
	_err =
	    buildDevInfDevInfCmd(pSubDecoder,
				 (VoidPtr_t) & pSubPcdata->content);
#endif

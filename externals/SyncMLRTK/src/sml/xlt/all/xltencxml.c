 ...
 *                  0, if OK
 */
Ret_t xmlGenerateTag(XltTagID_t tagId, XltTagType_t tagType,
		     BufferMgmtPtr_t pBufMgr, SmlPcdataExtension_t attFlag)
{
	...
	if ((_tagstr = (String_t) smlLibMalloc(XML_MAX_TAGLEN)) == NULL)
	...
	if (attFlag != pBufMgr->smlActiveExt || tagId == TN_SYNCML) {
		...
		if (getExtName(attFlag, &_tagnsattr, pBufMgr->vers) !=
		    SML_ERR_OK) {
			smlLibFree(_tagstr);
			return SML_ERR_XLT_INVAL_TAG_TYPE;
		}
	...
	case TT_END:
		{
			if ((_err =
				...
				break;
			if ((_err =
			     xltAddToBuffer((MemPtr_t) (&_tagdel), 1,
					    pBufMgr)) != SML_ERR_OK)
				break;
			if ((_err =
			     xltAddToBuffer((MemPtr_t) _tagstr,
					    smlLibStrlen(_tagstr),
					    pBufMgr)) != SML_ERR_OK)
				break;
			if ((_err =
	...
	case TT_ALL:
		{
			...
			if (_tagnsattr) {
				if ((_err =
				     xltAddToBuffer((MemPtr_t) (&_nstagstart),
						    8, pBufMgr)) != SML_ERR_OK)
					break;
				if ((_err =
				     xltAddToBuffer((MemPtr_t) _tagnsattr,
						    smlLibStrlen(_tagnsattr),
						    pBufMgr)) != SML_ERR_OK)
					break;
				if ((_err =
				     xltAddToBuffer((MemPtr_t) & _nstagend, 1,
						    pBufMgr)) != SML_ERR_OK)
					break;
			}
			if ((_err =
	...
	case TT_BEG:
		{
			...
			if (_tagnsattr) {
				if ((_err =
				     xltAddToBuffer((MemPtr_t) & _nstagstart, 8,
						    pBufMgr)) != SML_ERR_OK)
					break;
				if ((_err =
				     xltAddToBuffer((MemPtr_t) _tagnsattr,

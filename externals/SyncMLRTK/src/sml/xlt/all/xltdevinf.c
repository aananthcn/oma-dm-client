...
Ret_t buildDevInfDevInfCmd(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if (*ppElem != NULL)
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC, pScanner,
				       "buildDevInfDevInfCmd");
	...
	case TN_DEVINF_DEVINF:
		...
		break;
	default:
		rc = SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC, pScanner,
				     "buildDevInfDevInfCmd");
	}
	if (rc != SML_ERR_OK) {
		smlLibFree(pElem);
...
Ret_t buildDevInfDevInfContent(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if (*ppElem != NULL)
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC, pScanner,
				       "buildDevInfDevInfContent");

	if ((pElem =
	     (SmlDevInfDevInfPtr_t) smlLibMalloc(sizeof(SmlDevInfDevInf_t))) ==
	    NULL)
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_DEVINF_DATASTORE:
			rc = buildDevInfDataStoreList(pDecoder,
						      (VoidPtr_t) & pElem->
						      datastore);
			break;
		case TN_DEVINF_CTCAP:
			...
			break;
		case TN_DEVINF_EXT:
			rc = buildDevInfExtList(pDecoder,
						(VoidPtr_t) & pElem->ext);
			break;
		...
		case TN_DEVINF_LARGEOBJECT:
			...
			break;
		default:
			rc = SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					     pScanner,
					     "buildDevInfDevInfContent");
		}
		if (rc != SML_ERR_OK) {
...
static Ret_t buildDevInfFilterCapList(XltDecoderPtr_t pDecoder,
				      VoidPtr_t * ppElem)
{
	...
	pFiltercap = (SmlDevInfFilterCapListPtr_t)
	    smlLibMalloc(sizeof(SmlDevInfFilterCapList_t));
	if (pFiltercap == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	smlLibMemset(pFiltercap, 0, sizeof(SmlDevInfFilterCapList_t));
	pFiltercap->data = (SmlDevInfFilterCapPtr_t)
	    smlLibMalloc(sizeof(SmlDevInfFilterCap_t));
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_DEVINF_CTTYPE:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pFiltercap->data->
					 cttype);
			break;
		case TN_DEVINF_VERCT:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pFiltercap->data->verct);
			break;
 ...
 keywordOrPropname:
			...
			pList =
			    (SmlPcdataListPtr_t)
			    smlLibMalloc(sizeof(SmlPcdataList_t));
			if (pList == NULL)
				return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t buildDevInfDataStoreCmd(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if (*ppElem != NULL)
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC, pScanner,
				       "buildDevInfDataStoreCmd");

	if ((pElem = (SmlDevInfDatastorePtr_t)
	     smlLibMalloc(sizeof(SmlDevInfDatastore_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_DEVINF_SOURCEREF:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pElem->sourceref);
			break;
		case TN_DEVINF_DISPLAYNAME:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pElem->displayname);
			break;
		case TN_DEVINF_MAXGUIDSIZE:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pElem->maxguidsize);
			break;
			/* other elements */
		case TN_DEVINF_RXPREF:
			if (pElem->rxpref != NULL) {
				...
				rc = buildDevInfXmitCmd(pDecoder,
							(VoidPtr_t) & dummyP);
		...
		case TN_DEVINF_TXPREF:
			...
			break;
		case TN_DEVINF_RX:
			rc = buildDevInfXmitList(pDecoder,
						 (VoidPtr_t) & pElem->rx);
			break;
		case TN_DEVINF_TX:
			rc = buildDevInfXmitList(pDecoder,
		...
		case TN_DEVINF_DSMEM:
			rc = buildDevInfDSMemCmd(pDecoder,
						 (VoidPtr_t) & pElem->dsmem);
			break;
		case TN_DEVINF_SYNCCAP:
			rc = buildDevInfSyncCapCmd(pDecoder,
						   (VoidPtr_t) & pElem->
						   synccap);
			break;
		...
		case TN_DEVINF_FILTERCAP:
			rc = buildDevInfFilterCapList(pDecoder,
						      (VoidPtr_t) & pElem->
						      filtercap);
			break;
		case TN_DEVINF_FILTERRX:
			rc = buildDevInfXmitList(pDecoder,
						 (VoidPtr_t) & pElem->filterrx);
			break;
...
Ret_t buildDevInfXmitCmd(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pXmit =
	     (SmlDevInfXmitPtr_t) smlLibMalloc(sizeof(SmlDevInfXmit_t))) ==
	    NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t buildDevInfXmitList(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pElem = (SmlDevInfXmitListPtr_t)
	     smlLibMalloc(sizeof(SmlDevInfXmitList_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t buildDevInfDataStoreList(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pElem = (SmlDevInfDatastoreListPtr_t)
	     smlLibMalloc(sizeof(SmlDevInfDatastoreList_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t buildDevInfExtList(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pElem =
	     (SmlDevInfExtListPtr_t) smlLibMalloc(sizeof(SmlDevInfExtList_t)))
	    == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
static Ret_t scanPropOrParamElement(XltDecoderPtr_t pDecoder,
				    SmlDevInfCTDataPtr_t pPropOrParam)
{
	...
	case TN_DEVINF_DATATYPE:
		rc = buildPCData(pDecoder,
				 (VoidPtr_t) & pPropOrParam->datatype);
		break;
	...
	case TN_DEVINF_MAXOCCUR:
		rc = buildPCData(pDecoder,
				 (VoidPtr_t) & pPropOrParam->maxoccur);
		break;
		/* Flags */
	...
	case TN_DEVINF_VALENUM:
		...
		// create new list element
		pValenum =
		    (SmlPcdataListPtr_t) smlLibMalloc(sizeof(SmlPcdataList_t));
		if (pValenum == NULL)
			return SML_ERR_NOT_ENOUGH_SPACE;
		...
		break;
	default:
		rc = SML_DECODEERROR(SML_ERR_XLT_INVAL_PROTO_ELEM, pScanner,
				     "scanPropOrParamElement");
	}
	return rc;
}
...
static Ret_t buildDevInfPropParam(XltDecoderPtr_t pDecoder,
				  VoidPtr_t * ppElem, int datastoreLocal)
{
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		if (!datastoreLocal
		    && pScanner->curtok->tagid == TN_DEVINF_PARAMNAME) {
		...
		if (newElement) {
			newElement = FALSE;
			pParam = (SmlDevInfCTDataListPtr_t)
			    smlLibMalloc(sizeof(SmlDevInfCTDataList_t));
			if (pParam == NULL)
				return SML_ERR_NOT_ENOUGH_SPACE;
			smlLibMemset(pParam, 0, sizeof(SmlDevInfCTDataList_t));
			pParam->data = (SmlDevInfCTDataPtr_t)
			    smlLibMalloc(sizeof(SmlDevInfCTData_t));
			if (pParam->data == NULL) {
				smlFreeDevInfCTDataList(pParam);
				return SML_ERR_NOT_ENOUGH_SPACE;
			}
			smlLibMemset(pParam->data, 0,
				     sizeof(SmlDevInfCTData_t));
			// link into list
			*ppListElem = pParam;
			// set elem link to where we must link in next element
		...
		if (pParam == NULL)
			return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					       pScanner,
					       "buildDevInfPropParam");
...
static Ret_t buildDevInfProperty(XltDecoderPtr_t pDecoder,
				 VoidPtr_t * ppElem, int datastoreLocal)
{
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		if (!datastoreLocal
		    && pScanner->curtok->tagid == TN_DEVINF_PROPNAME) {
		...
		if (newElement) {
			newElement = FALSE;
			pProp = (SmlDevInfCTDataPropListPtr_t)
			    smlLibMalloc(sizeof(SmlDevInfCTDataPropList_t));
			if (pProp == NULL)
				return SML_ERR_NOT_ENOUGH_SPACE;
			smlLibMemset(pProp, 0,
				     sizeof(SmlDevInfCTDataPropList_t));
			pProp->data = (SmlDevInfCTDataPropPtr_t)
			    smlLibMalloc(sizeof(SmlDevInfCTDataProp_t));
			if (pProp->data == NULL) {
				smlFreeDevInfCTDataPropList(pProp);
				return SML_ERR_NOT_ENOUGH_SPACE;
			}
			smlLibMemset(pProp->data, 0,
				     sizeof(SmlDevInfCTDataProp_t));
			pProp->data->prop = (SmlDevInfCTDataPtr_t)
			    smlLibMalloc(sizeof(SmlDevInfCTData_t));
			if (pProp->data->prop == NULL) {
				smlFreeDevInfCTDataPropList(pProp);
				return SML_ERR_NOT_ENOUGH_SPACE;
			}
			smlLibMemset(pProp->data->prop, 0,
				     sizeof(SmlDevInfCTData_t));
			// link into list
			*ppListElem = pProp;
			// set elem link to where we must link in next element
		...
		if (pProp == NULL)
			return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					       pScanner, "buildDevInfProperty");

		// process tags
		switch (pScanner->curtok->tagid) {
		case TN_DEVINF_PROPPARAM:
			// DS 1.2 case: only </PropParam> ends the param building process
			rc = buildDevInfPropParam(pDecoder,
						  (VoidPtr_t) & pProp->data->
						  param, datastoreLocal);
			break;
		case TN_DEVINF_PARAMNAME:
		...
		case TN_DEVINF_PROPNAME:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pProp->data->prop->name);
			break;

		default:
			rc = scanPropOrParamElement(pDecoder,
						    pProp->data->prop);
			if (rc == SML_ERR_XLT_INVAL_PROTO_ELEM
			    && !datastoreLocal)
				return SML_ERR_OK;	// parser signals unknown tag, this is ok for pre-1.2
		       ...
		       Boolean_t datastoreLocal)
{
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		if (!datastoreLocal
		    && pScanner->curtok->tagid == TN_DEVINF_CTTYPE) {
		...
		if (newElement) {
			newElement = FALSE;
			pCtcap = (SmlDevInfCtcapListPtr_t)
			    smlLibMalloc(sizeof(SmlDevInfCtcapList_t));
			if (pCtcap == NULL)
				return SML_ERR_NOT_ENOUGH_SPACE;
			smlLibMemset(pCtcap, 0, sizeof(SmlDevInfCtcapList_t));
			pCtcap->data = (SmlDevInfCTCapPtr_t)
			    smlLibMalloc(sizeof(SmlDevInfCTCap_t));
			if (pCtcap->data == NULL) {
		...
		if (pCtcap == NULL)
			return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					       pScanner, "buildDevInfCtcap");
		// process tags
		switch (pScanner->curtok->tagid) {
		case TN_DEVINF_CTTYPE:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pCtcap->data->cttype);
			break;
		case TN_DEVINF_VERCT:
			rc = buildPCData(pDecoder,
					 (VoidPtr_t) & pCtcap->data->verct);
			break;
...
Ret_t buildDevInfDSMemCmd(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pElem =
	     (SmlDevInfDSMemPtr_t) smlLibMalloc(sizeof(SmlDevInfDSMem_t))) ==
	    NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t buildDevInfCTCapCmd(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pElem =
	     (SmlDevInfCTCapPtr_t) smlLibMalloc(sizeof(SmlDevInfCTCap_t))) ==
	    NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
...
Ret_t buildDevInfSyncCapCmd(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if (*ppElem != NULL)
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC, pScanner,
				       "buildDevInfSyncCapCmd");

	if ((pElem =
	     (SmlDevInfSyncCapPtr_t) smlLibMalloc(sizeof(SmlDevInfSyncCap_t)))
	    == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_DEVINF_SYNCTYPE:
			rc = buildPCDataList(pDecoder,
					     (VoidPtr_t) & pElem->synctype);
			break;
		default:
			rc = SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					     pScanner, "buildDevInfSyncCapCmd");
		}
		if (rc != SML_ERR_OK) {
			smlLibFree(pElem);
			return rc;
...
Ret_t buildDevInfExtCmd(XltDecoderPtr_t pDecoder, VoidPtr_t * ppElem)
{
	...
	if ((pElem =
	     (SmlDevInfExtPtr_t) smlLibMalloc(sizeof(SmlDevInfExt_t))) == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	...
	while (pScanner->curtok->type != TOK_TAG_END) {
		...
		case TN_DEVINF_XVAL:
			rc = buildPCDataList(pDecoder,
					     (VoidPtr_t) & pElem->xval);
			break;
		default:
			rc = SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					     pScanner, "buildDevInfExtCmd");
		}
		if (rc != SML_ERR_OK) {
			smlLibFree(pElem);
			return rc;
...
Ret_t devinfEncBlock(XltTagID_t tagId, XltRO_t reqOptFlag,
		     ...
		     BufferMgmtPtr_t pBufMgr, SmlPcdataExtension_t attFlag)
{
	...
	case TN_DEVINF_EXT:
		if ((_err =
		     xltGenerateTag(TN_DEVINF_EXT, TT_BEG, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_XNAM, REQUIRED,
				    ((SmlDevInfExtPtr_t) pContent)->xnam, enc,
				    pBufMgr, SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		pList = ((SmlDevInfExtPtr_t) pContent)->xval;
		while (pList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_XVAL, OPTIONAL,
					    pList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			pList = pList->next;
		};
		if ((_err =
		     xltGenerateTag(TN_DEVINF_EXT, TT_END, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		break;
	case TN_DEVINF_SYNCCAP:
		if ((_err =
		     xltGenerateTag(TN_DEVINF_SYNCCAP, TT_BEG, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		pList = ((SmlDevInfSyncCapPtr_t) pContent)->synctype;
		while (pList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_SYNCTYPE, OPTIONAL,
					    pList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			pList = pList->next;
		...
		if ((_err =
		     xltGenerateTag(TN_DEVINF_SYNCCAP, TT_END, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		break;
	case TN_DEVINF_SHAREDMEM:
 ...
 param_or_property:
		...
		if ((_err =
		     devinfEncBlock(nameTagID, REQUIRED, pPropOrParam->name,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		...
		if (pPropOrParam->valenum != NULL
		    && pPropOrParam->datatype != NULL
		    && pPropOrParam->datatype->length != 0)
		...
		if (pPropOrParam->valenum != NULL) {
			...
			while (pList != NULL) {
				if ((_err =
				     devinfEncBlock(TN_DEVINF_VALENUM, REQUIRED,
						    pList->data, enc, pBufMgr,
						    SML_EXT_DEVINF)) !=
				    SML_ERR_OK)
					return _err;
			...
			// Datatype?
			if ((_err =
			     devinfEncBlock(TN_DEVINF_DATATYPE, REQUIRED,
					    pPropOrParam->datatype, enc,
					    pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
		}
		...
		if ((_err =
		     devinfEncBlock(pBufMgr->vers >=
				    SML_VERS_1_2 ? TN_DEVINF_MAXSIZE :
				    TN_DEVINF_SIZE, OPTIONAL,
				    pPropOrParam->maxsize, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		/* -- DS 1.2: MaxOccur?, is valid for Properties only, not for PropParams */
		if ((_err =
		     devinfEncBlock(TN_DEVINF_MAXOCCUR, OPTIONAL,
				    pPropOrParam->maxoccur, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		/* -- DS 1.2: (NoTruncate?), is valid for Properties only, not for PropParams */
		if ((_err =
		     devinfEncBlock(TN_DEVINF_NOTRUNCATE, OPTIONAL,
				    &(pPropOrParam->flags), enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;

		/* -- DisplayName ? */
		if ((_err =
		     devinfEncBlock(TN_DEVINF_DISPLAYNAME, OPTIONAL,
				    pPropOrParam->dname, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		...
		while (paramList != NULL) {
			...
			if ((_err =
			     devinfEncBlock(TN_DEVINF_PROPPARAM, REQUIRED,
					    paramList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
	...
	case TN_DEVINF_CTCAP:
		...
		if (pBufMgr->vers < SML_VERS_1_2) {
			if ((_err =
			     xltGenerateTag(TN_DEVINF_CTCAP, TT_BEG, enc,
					    pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
		}
		while (ctList != NULL) {
			...
			if (pBufMgr->vers >= SML_VERS_1_2) {
				if ((_err =
				     xltGenerateTag(TN_DEVINF_CTCAP, TT_BEG,
						    enc, pBufMgr,
						    SML_EXT_DEVINF)) !=
				    SML_ERR_OK)
					return _err;
			...
			if ((_err =
			     devinfEncBlock(TN_DEVINF_CTTYPE, REQUIRED,
					    ctList->data->cttype, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			/* SyncML DS 1.2, Synthesis/luz 2005-09-28, required for DS 1.2 only, therefore OPTIONAL here */
			if ((_err =
			     devinfEncBlock(TN_DEVINF_VERCT, OPTIONAL,
					    ctList->data->verct, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			/* SyncML DS 1.2, Synthesis/luz 2005-08-24 */
			if ((_err =
			     devinfEncBlock(TN_DEVINF_FIELDLEVEL, OPTIONAL,
					    &(ctList->data->flags), enc,
					    pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			...
			while (propList != NULL) {
				...
				if ((_err =
				     devinfEncBlock(TN_DEVINF_PROPERTY,
						    REQUIRED, propList->data,
						    enc, pBufMgr,
						    SML_EXT_DEVINF)) !=
				    SML_ERR_OK)
					return _err;
			...
			if (pBufMgr->vers >= SML_VERS_1_2) {
				if ((_err =
				     xltGenerateTag(TN_DEVINF_CTCAP, TT_END,
						    enc, pBufMgr,
						    SML_EXT_DEVINF)) !=
				    SML_ERR_OK)
					return _err;
		...
		if (pBufMgr->vers < SML_VERS_1_2) {
			if ((_err =
			     xltGenerateTag(TN_DEVINF_CTCAP, TT_END, enc,
					    pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
		}
	...
	case TN_DEVINF_DSMEM:
		if ((_err =
		     xltGenerateTag(TN_DEVINF_DSMEM, TT_BEG, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_SHAREDMEM, OPTIONAL,
				    &(((SmlDevInfDSMemPtr_t) pContent)->flags),
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_MAXMEM, OPTIONAL,
				    ((SmlDevInfDSMemPtr_t) pContent)->maxmem,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_MAXID, OPTIONAL,
				    ((SmlDevInfDSMemPtr_t) pContent)->maxid,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     xltGenerateTag(TN_DEVINF_DSMEM, TT_END, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		break;
	...
	case TN_DEVINF_FILTERRX:	/* SyncML DS 1.2, Synthesis/luz 2005-08-24 */
		...
		if ((_err =
		     devinfEncBlock(TN_DEVINF_CTTYPE, REQUIRED,
				    ((SmlDevInfXmitPtr_t) pContent)->cttype,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_VERCT, REQUIRED,
	...
	case TN_DEVINF_FILTERCAP:
		...
		if ((_err =
		     devinfEncBlock(TN_DEVINF_CTTYPE, REQUIRED,
				    ((SmlDevInfFilterCapPtr_t) pContent)->
				    cttype, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_VERCT, REQUIRED,
				    ((SmlDevInfFilterCapPtr_t) pContent)->verct,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		...
		while (pList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_FILTERKEYWORD, REQUIRED,
					    pList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			pList = pList->next;
		...
		while (pList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_PROPNAME, REQUIRED,
					    pList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			pList = pList->next;
	...
	case TN_DEVINF_DATASTORE:
		if ((_err =
		     xltGenerateTag(TN_DEVINF_DATASTORE, TT_BEG, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_SOURCEREF, REQUIRED,
				    ((SmlDevInfDatastorePtr_t)
				     pContent)->sourceref, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_DISPLAYNAME, OPTIONAL,
				    ((SmlDevInfDatastorePtr_t)
				     pContent)->displayname, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_MAXGUIDSIZE, OPTIONAL,
				    ((SmlDevInfDatastorePtr_t)
				     pContent)->maxguidsize, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_RXPREF, REQUIRED,
				    ((SmlDevInfDatastorePtr_t) pContent)->
				    rxpref, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		...
		while (xmList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_RX, OPTIONAL,
					    xmList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			xmList = xmList->next;
		};
		if ((_err =
		     devinfEncBlock(TN_DEVINF_TXPREF, REQUIRED,
				    ((SmlDevInfDatastorePtr_t) pContent)->
				    txpref, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		...
		while (xmList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_TX, OPTIONAL,
					    xmList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			xmList = xmList->next;
		...
		if ((_err =
		     devinfEncBlock(TN_DEVINF_CTCAP, OPTIONAL,
				    ((SmlDevInfDatastorePtr_t) pContent)->ctcap,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;

		if ((_err =
		     devinfEncBlock(TN_DEVINF_DSMEM, OPTIONAL,
				    ((SmlDevInfDatastorePtr_t) pContent)->dsmem,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		/* SyncML DS 1.2, Synthesis/luz 2005-08-24 */
		if ((_err =
		     devinfEncBlock(TN_DEVINF_HIERARCHICAL, OPTIONAL,
				    &(((SmlDevInfDatastorePtr_t) pContent)->
				      flags), enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		...
		if ((_err =
		     devinfEncBlock(TN_DEVINF_SYNCCAP, REQUIRED,
				    ((SmlDevInfDatastorePtr_t) pContent)->
				    synccap, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		...
		while (xmList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_FILTERRX, OPTIONAL,
					    xmList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			xmList = xmList->next;
		...
		while (fcapList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_FILTERCAP, OPTIONAL,
					    fcapList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
		...
		if ((_err =
		     xltGenerateTag(TN_DEVINF_DATASTORE, TT_END, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		break;
	...
	case TN_DEVINF_DEVINF:
		if ((_err =
		     xltGenerateTag(TN_DEVINF_DEVINF, TT_BEG, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_VERDTD, REQUIRED,
				    ((SmlDevInfDevInfPtr_t) pContent)->verdtd,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_MAN, OPTIONAL,
				    ((SmlDevInfDevInfPtr_t) pContent)->man, enc,
				    pBufMgr, SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_MOD, OPTIONAL,
				    ((SmlDevInfDevInfPtr_t) pContent)->mod, enc,
				    pBufMgr, SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_OEM, OPTIONAL,
				    ((SmlDevInfDevInfPtr_t) pContent)->oem, enc,
				    pBufMgr, SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_FWV, OPTIONAL,
				    ((SmlDevInfDevInfPtr_t) pContent)->fwv, enc,
				    pBufMgr, SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_SWV, OPTIONAL,
				    ((SmlDevInfDevInfPtr_t) pContent)->swv, enc,
				    pBufMgr, SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_HWV, OPTIONAL,
				    ((SmlDevInfDevInfPtr_t) pContent)->hwv, enc,
				    pBufMgr, SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_DEVID, REQUIRED,
				    ((SmlDevInfDevInfPtr_t) pContent)->devid,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_DEVTYP, REQUIRED,
				    ((SmlDevInfDevInfPtr_t) pContent)->devtyp,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_UTC, OPTIONAL,
				    &(((SmlDevInfDevInfPtr_t) pContent)->flags),
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_NOFM, OPTIONAL,
				    &(((SmlDevInfDevInfPtr_t) pContent)->flags),
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		if ((_err =
		     devinfEncBlock(TN_DEVINF_LARGEOBJECT, OPTIONAL,
				    &(((SmlDevInfDevInfPtr_t) pContent)->flags),
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		...
		if (dsList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_DATASTORE, REQUIRED,
					    dsList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			dsList = dsList->next;
			while (dsList != NULL) {
				if ((_err =
				     devinfEncBlock(TN_DEVINF_DATASTORE,
						    OPTIONAL, dsList->data, enc,
						    pBufMgr,
						    SML_EXT_DEVINF)) !=
				    SML_ERR_OK)
					return _err;
		...
		if ((_err =
		     devinfEncBlock(TN_DEVINF_CTCAP, OPTIONAL,
				    ((SmlDevInfDevInfPtr_t) pContent)->ctcap,
				    enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		// extensions
		...
		while (exList != NULL) {
			if ((_err =
			     devinfEncBlock(TN_DEVINF_EXT, OPTIONAL,
					    exList->data, enc, pBufMgr,
					    SML_EXT_DEVINF)) != SML_ERR_OK)
				return _err;
			exList = exList->next;
		};
		...
		if ((_err =
		     xltGenerateTag(TN_DEVINF_DEVINF, TT_END, enc, pBufMgr,
				    SML_EXT_DEVINF)) != SML_ERR_OK)
			return _err;
		break;

	default:{		// all leaf nodes (PCDATA#)
			return xltEncPcdata(tagId, reqOptFlag, pContent, enc,
					    pBufMgr, attFlag);

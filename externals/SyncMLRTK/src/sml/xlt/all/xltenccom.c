 ...
 *                 0, if OK
 */
Ret_t xltAddToBuffer(const MemPtr_t pContent, MemSize_t size,
		     BufferMgmtPtr_t pBufMgr)
{
	...
	if (!pBufMgr->spaceEvaluation) {
		...
		if ((size + pBufMgr->smlXltWrittenBytes) >
...
#warning "%%%%% delete that message later"
		...
		if (!
		    (smlLibMemcpy
		     ((void *)pBufMgr->smlXltBufferP, (void *)pContent,
		      (MemSize_t) size))) {
#ifdef NCDEBUGPRINTFX
#warning "%%%%% delete that message later"
			NCDEBUGPRINTFX(DBG_RTK_SML,
				       ("xltAddToBuffer: memCpy failed, pBufMgr->smlXltBufferP=%lX, size=%ld",
					(long)pBufMgr->smlXltBufferP, size));
#endif
			return SML_ERR_XLT_BUF_ERR;
		}

...
Ret_t xltUtilCreateStack(XltUtilStackPtr_t * ppStack, const Long_t size)
{
	...
	if ((pStack =
	     (ArrayStackPtr_t) smlLibMalloc(sizeof(ArrayStack_t))) == NULL) {
		*ppStack = NULL;
	...
	pStack->array = NULL;
	if ((pStack->array =
	     (XltUtilStackItem_t *) smlLibMalloc(size *
						 sizeof(XltUtilStackItem_t)))
	    == NULL) {
		*ppStack = NULL;
		smlLibFree(pStack);
		return SML_ERR_NOT_ENOUGH_SPACE;
...
static Ret_t _top(const XltUtilStackPtr_t pStack, XltUtilStackItem_t * itemPtr)
{
...
static Ret_t _pop(XltUtilStackPtr_t pStack, XltUtilStackItem_t * itemPtr)
{
	ArrayStackPtr_t pStackPriv = (ArrayStackPtr_t) pStack;
	...
	if ((pStackPriv->topidx >= 0) &&
		...
		if ((newarray =
		     (XltUtilStackItem_t *) smlLibRealloc(pStackPriv->array,
							  newsize *
							  sizeof
							  (XltUtilStackItem_t)))
		    != NULL) {
...
static Ret_t _push(XltUtilStackPtr_t pStack, const XltUtilStackItem_t item)
{
	ArrayStackPtr_t pStackPriv = (ArrayStackPtr_t) pStack;
...
static Ret_t _destroy(XltUtilStackPtr_t pStack)
{

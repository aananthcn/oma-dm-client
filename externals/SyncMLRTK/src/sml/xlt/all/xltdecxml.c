...
struct xmlScannerPriv_s {
	...
	void (*setBuf) (XltDecScannerPtr_t pScanner, const MemPtr_t pBufStart,
	...
	SmlPcdataExtension_t prev_ext;
				   /**< which is the previous open namespace ? */
...
static Ret_t xmlAttribute(xmlScannerPrivPtr_t pScanner, String_t * name,
			  String_t * value);
...
Ret_t
xltDecXmlInit(const MemPtr_t pBufEnd, MemPtr_t * ppBufPos,
	      XltDecScannerPtr_t * ppScanner)
{
	...
	pScanner->curtok =
	    (XltDecTokenPtr_t) smlLibMalloc(sizeof(XltDecToken_t));
 ...
 */
static Ret_t _destroy(XltDecScannerPtr_t pScanner)
{
 ...
 */
static Ret_t _nextTok(XltDecScannerPtr_t pScanner)
{
	xmlScannerPrivPtr_t pScannerPriv;
	Ret_t rc;
	...
	while (!rc) {
		if (smlLibStrncmp((String_t) pScannerPriv->pos, "<!--", 4) == 0) {
		...
		} else if (smlLibStrncmp((String_t) pScannerPriv->pos, "<?", 2)
			   == 0) {
		    ...
		    if (smlLibStrncmp
			((String_t) pScannerPriv->pos, "<![CDATA[", 9)
			== 0) {
			rc = xmlCDATA(pScannerPriv);
			break;
		} else if ((isPcdata(pScannerPriv->curtok->tagid))
			   && (pScannerPriv->curtok->type != TOK_TAG_END)) {
			rc = xmlSkipPCDATA(pScannerPriv);
			break;
		} else if (smlLibStrncmp((String_t) pScannerPriv->pos, "<", 1)
			   == 0) {
 ...
 */
static MemPtr_t _getPos(XltDecScannerPtr_t pScanner)
{
...
static Ret_t bomDecl(xmlScannerPrivPtr_t pScanner)
{
	...
	if (bom0 != 0x00 && bom1 != 0x00 && bom0 < 0xfe && bom1 < 0xfe) {	//utf-8

	/** check utf-8 BOM */
		if (bom0 == 0xef && bom1 == 0xbb && bom2 == 0xbf)
			bomLength = 3;
		//else no bom
	} else if (bom0 == 0x00 && bom1 == 0x00 && bom2 == 0xfe && bom3 == 0xff) {	//utf-32(be)
		return SML_ERR_XLT_ENC_UNK;
	} else if (bom0 == 0xff && bom1 == 0xfe && bom2 == 0x00 && bom3 == 0x00) {	//utf-32(le)
		return SML_ERR_XLT_ENC_UNK;
...
static Ret_t xmlDocTypeDecl(xmlScannerPrivPtr_t pScanner)
{
	...
	if ((pScanner->pos + 6 <= pScanner->bufend) &&
		...
		if ((rc = xmlStringConst(pScanner, &syslit)) != SML_ERR_OK) {
		   ...
		   (smlLibStrncmp((String_t) pScanner->pos, "PUBLIC", 6) ==
		    0)) {
		readBytes(pScanner, 6);
		skipS(pScanner);
	...
	if (*pScanner->pos != '>')
		return SML_DECODEERROR(SML_ERR_XLT_INVAL_XML_DOC, pScanner,
				       "xmlDocTypeDecl");
...
static Ret_t xmlXMLDecl(xmlScannerPrivPtr_t pScanner)
{
	...
	if (pScanner->pos + 2 > pScanner->bufend)
		return SML_DECODEERROR(SML_ERR_XLT_END_OF_BUFFER, pScanner,
				       "xmlXMLDecl");
...
xmlAttribute(xmlScannerPrivPtr_t pScanner, String_t * name, String_t * value)
{
	...
	if (*pScanner->pos == '>') {
		return SML_DECODEERROR(SML_ERR_XLT_MISSING_CONT, pScanner,
				       "xmlAttribute");
	}
	...
	while (*pScanner->pos != ';') {
		if (pScanner->pos >= pScanner->bufend) {
			return SML_DECODEERROR(SML_ERR_XLT_INVAL_SYNCML_DOC,
					       pScanner, "xmlHTMLEntity");
		}
		if (!readBytes(pScanner, 1)) {
			return SML_DECODEERROR(SML_ERR_XLT_END_OF_BUFFER,
					       pScanner, "xmlHTMLEntity");
		}
	}
	...
	if (*pScanner->pos == '&') {
		...
		while (*pScanner->pos != '<' && (*pScanner->pos != '&')) {
			...
			if (!readBytes(pScanner, 1)) {
				smlLibFree(pPCData);
				return
				    SML_DECODEERROR(SML_ERR_XLT_END_OF_BUFFER,
						    pScanner, "xmlCharData");
			}
		}
...
static Ret_t xmlTag(xmlScannerPrivPtr_t pScanner, Byte_t endtag)
{
	...
	if ((rc = xmlName(pScanner, &name)) != SML_ERR_OK) {
		if (rc != SML_ERR_NOT_ENOUGH_SPACE) {
			return SML_DECODEERROR(SML_ERR_XLT_INVAL_XML_DOC,
					       pScanner, "xmlTag");
		} else {
			return rc;
		}
	...
	if (!endtag) {
		...
		if ((rc =
		     xmlAttribute(pScanner, &attname, &value)) == SML_ERR_OK) {
			if (smlLibStrncmp(attname, "xmlns", 5) == 0) {
				...
				if (rc == SML_ERR_NOT_ENOUGH_SPACE) {
					...
					return
					    SML_DECODEERROR
					    (SML_ERR_XLT_INVAL_XML_DOC,
					     pScanner, "xmlTag");
				}
			}
		} else if (rc !=
			   SML_DECODEERROR(SML_ERR_XLT_MISSING_CONT, pScanner,
					   "xmlTag")) {
	...
	if (pScanner->ext == ext) {
		...
		if (pScanner->nsprelen > 0
		    && smlLibStrlen(name) > pScanner->nsprelen + 1) {
			if (name[pScanner->nsprelen] != ':'
			    || smlLibStrncmp(name, pScanner->nsprefix,
					     pScanner->nsprelen) != 0) {
				smlLibFree(name);
				smlLibFree(attname);
				smlLibFree(value);
				...
				return
				    SML_DECODEERROR
				    (SML_ERR_XLT_NO_MATCHING_CODEPAGE, pScanner,
				     "xmlTag");
			}
		}
		 ...
		 */
		if (pScanner->nsprelen > 0)
			rc = getTagIDByStringAndExt(&name
						    [0 + pScanner->nsprelen +
						     1], pScanner->ext, &tagid);
		else
			rc = getTagIDByStringAndExt(name, pScanner->ext,
						    &tagid);
		...
		if (nsprelen > 0 && smlLibStrlen(name) > nsprelen + 1) {
			if (name[nsprelen] != ':'
			    || smlLibStrncmp(name, nsprefix, nsprelen) != 0) {
				smlLibFree(name);
				smlLibFree(attname);
				smlLibFree(value);
				smlLibFree(nsprefix);
		 ...
		 */
		if (nsprelen > 0)
			rc = getTagIDByStringAndExt(&name[nsprelen + 1], ext,
						    &tagid);
		else
	...
	if (endtag) {
		...
		if ((rc = xmlSkipAttributes(pScanner)) != SML_ERR_OK)
			return rc;
...
static Ret_t xmlSkipAttributes(xmlScannerPrivPtr_t pScanner)
{
	...
	while ((pScanner->pos + 1 <= pScanner->bufend) &&
	       (smlLibStrncmp((String_t) pScanner->pos, ">", 1))
	       && (smlLibStrncmp((String_t) pScanner->pos, "/>", 2)))
		if (!readBytes(pScanner, 1))
			return SML_DECODEERROR(SML_ERR_XLT_END_OF_BUFFER,
					       pScanner, "xmlSkipAttributes");
...
static Ret_t xmlSkipPCDATA(xmlScannerPrivPtr_t pScanner)
{
	...
	if (_tagString == NULL)
		return SML_ERR_NOT_ENOUGH_SPACE;
	if ((rc =
	     getTagString(pScanner->curtok->tagid, _tagString,
	...
	_tagString2 =
	    smlLibMalloc(smlLibStrlen(_tagString) + 4 +
			 (pScanner->nsprelen + 1));
	...
	if (*pScanner->pos == '&') {
		...
		while (smlLibStrncmp
		       ((String_t) pScanner->pos, _tagString2,
			smlLibStrlen(_tagString2)) != 0) {
			...
			if (smlLibStrncmp
			    ((String_t) pScanner->pos, "<![CDATA[", 9) == 0
			    || *pScanner->pos == '&') {

...
Ret_t wbxmlOpaqueSize2Buf(Long_t size, BufferMgmtPtr_t pBufMgr)
{
	...
	if (pBufMgr->spaceEvaluation == 0) {
		...
		if ((_byteArray = smlLibMalloc(_bytesNeeded)) == NULL)
 ...
 *                  0, if OK
 */
Ret_t wbxmlGenerateTag(XltTagID_t tagId, XltTagType_t tagType,
		       BufferMgmtPtr_t pBufMgr)
{
	...
	case TT_ALL:
		{
			_err =
			    (MemByte_t) getTagByte(tagId, pBufMgr->smlCurExt,
						   &_tmp);
 ...
 *                  0, if OK
 */
Ret_t wbxmlWriteTypeToBuffer(const MemPtr_t pContent,
			     XltElementType_t elType, Long_t size,
			     BufferMgmtPtr_t pBufMgr)
{
	Ret_t _err;
	...
	case STR_I:
		{
			...
			if ((_err =
			     xltAddToBuffer(&_tmp, 1, pBufMgr)) != SML_ERR_OK)
				return _err;

			//add the string to the buffer
			if ((_err =
			     xltAddToBuffer(pContent,
					    (!pContent) ? 0 :
	...
	case OPAQUE:
		{
			...
			if ((_err =
			     wbxmlOpaqueSize2Buf(size, pBufMgr)) != SML_ERR_OK)
				return _err;

			//add the string buffer
			if ((_err =
			     xltAddToBuffer(pContent, size,
					    pBufMgr)) != SML_ERR_OK)
				return _err;

...
typedef struct WsmOptions_s {
	MemSize_t maxAvailMem;
			   /**< maximum amount of memory available for all wsm buffers */
} WsmOptions_t;
...
typedef struct WsmBuf_s {
	...
	MemPtr_t pFirstFree;
			   /**< pointer to first free element in buffer */
	MemPtr_t pFirstData;
			   /**< pointer to first data element in buffer */
...
Ret_t wsmCreate(String_t bufName, MemSize_t bufSize,
		MemHandle_t * wsmH) WSM_FUNC;
